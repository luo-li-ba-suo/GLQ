...
from offpolicy.algorithms.glq_Qbias.algorithm.q_global import q_global
...
class GLQ(Trainer):
    ...
    def train_policy_on_batch(self, batch, update_policy_id=None, num_agents=None):
        ...
        for p_id in self.policy_ids:
            ...
            if self.add_other_act_to_cent:
                Q_global_seq = self.global_q(agent_q_seq, stacked_cent_obs_batch[:-1], curr_other_act_batch).unsqueeze(-1)
                next_step_Q_global_seq = self.target_global_q(agent_nq_seq, stacked_cent_obs_batch[1:], next_other_act_batch).unsqueeze(-1)
            else:

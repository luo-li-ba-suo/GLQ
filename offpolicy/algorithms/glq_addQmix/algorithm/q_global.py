...
class q_global(nn.Module):
    ...
    def __init__(self, args, num_agents, cent_obs_dim, act_dim, device, multidiscrete_list=None):
        ...
        self.hyper_b3 = init_(nn.Linear(self.cent_obs_dim, self.hypernet_output_dim))
    ...
    def forward(self, agent_q_individual, states, actions=None):
        ...
        w3 = w3.view(-1, batch_size, 1, self.num_q_inps, self.hidden_layer_dim)
        b3 = b3.view(-1, batch_size, 1, 1, self.hidden_layer_dim)
        w4 = w4.view(-1, batch_size, 1, self.hidden_layer_dim, 1)
        ...
        hidden_layer = F.elu(torch.matmul(agent_q_inps, w3) + b3)
        out2 = torch.matmul(hidden_layer, w4) + b4
        q_tot = out2.view(-1, batch_size)
        return q_global, q_tot

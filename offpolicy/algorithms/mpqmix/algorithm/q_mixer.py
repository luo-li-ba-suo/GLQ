...
class QMixer(nn.Module):
    ...
    def __init__(self, args, num_agents, cent_obs_dim, device, multidiscrete_list=None):
        ...
        # multi-head QMix
        self.num_perspective = args.num_perspective
        self.resort_q = args.ablation_resort_q
    ...
    def forward(self, agent_q_inps, states):
        ...
        states = states.view(-1, batch_size, self.num_perspective, self.cent_obs_dim).float()
        agent_q_inps = agent_q_inps.view(-1, batch_size, 1, 1, self.num_mixer_q_inps)
        if not self.resort_q:
            agent_q_inps = agent_q_inps.repeat(1, 1, self.num_perspective, 1, 1)
        else:
            agent_q_inps = torch.cat([agent_q_inps[..., order] for order in self.state_resort_orders], dim=-3)
        w1 = torch.abs(self.hyper_w1(states))
        b1 = self.hyper_b1(states)
        w1 = w1.view(-1, batch_size, self.num_perspective, self.num_mixer_q_inps, self.hidden_layer_dim)
        b1 = b1.view(-1, batch_size, self.num_perspective, 1, self.hidden_layer_dim)
        hidden_layer = F.elu(torch.matmul(agent_q_inps, w1) + b1)

        w2 = torch.abs(self.hyper_w2(states))
        b2 = self.hyper_b2(states)
        w2 = w2.view(-1, batch_size, self.num_perspective, self.hidden_layer_dim, 1)
        b2 = b2.view(-1, batch_size, self.num_perspective, 1, 1)
        out = torch.matmul(hidden_layer, w2) + b2
        q_tot = out.view(-1, batch_size, self.num_perspective)
        return q_tot

...
from offpolicy.algorithms.glq_mixLayer.algorithm.q_global import q_global
    ...
    def train_policy_on_batch(self, batch, update_policy_id=None, num_agents=None):
        ...
        for p_id in self.policy_ids:
            ...
            if self.use_huber_loss:
                loss1 = huber_loss(error1, self.huber_delta).sum() / (1 - bad_transitions_mask).sum()
            else:
                loss1 = mse_loss(error1).sum() / (1 - bad_transitions_mask).sum()
            if self.use_huber_loss:

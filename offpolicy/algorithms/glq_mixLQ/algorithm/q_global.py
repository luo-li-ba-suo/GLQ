...
class q_global(nn.Module):
    ...
    def __init__(self, args, num_agents, cent_obs_dim, act_dim, device, multidiscrete_list=None):
        ...
        if self.use_same_share_obs:  # 如果不使用同一个global net且使用同一个global state
    ...
    def forward(self, agent_q_individual, states, actions=None):
        ...
        elif self.add_other_act_to_cent:
            states = torch.cat((states, actions), dim=-1).view(-1, batch_size, self.num_q_inps, self.cent_obs_dim).float()
        else:
        ...
        if self.use_same_share_obs or self.share_hyper_network:
        ...
        elif not self.use_same_share_obs and not self.share_hyper_network:

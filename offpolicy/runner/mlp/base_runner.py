...
class MlpRunner(object):
    ...
    def __init__(self, config):
        ...
        elif self.algorithm_name in self.global_and_local_q:
    ...
    def save_glq(self):
        ...
        for pid in self.policy_ids:
            ...
            global_q = self.policies[pid].global_q
            gq_save_path = self.save_dir + '/' + str(pid)
    ...
    def restore_glq(self):
        ...
        for pid in self.policy_ids:
            path = str(self.model_dir) + str(pid)
            print("load the pretrained model from {}".format(path))
            global_q_state_dict = torch.load(path + '/global_q.pt')
            policy_q_state_dict = torch.load(path + '/q_network.pt')

            self.policies[pid].global_q.load_state_dict(global_q_state_dict)
            self.policies[pid].q_network.load_state_dict(policy_q_state_dict)

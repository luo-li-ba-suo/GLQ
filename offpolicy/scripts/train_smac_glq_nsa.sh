...
for map in $maps; do
    for seed in $seeds; do
        ...
        echo "seed is ${seed}:"
        pip3 install -e ../..
        CUDA_VISIBLE_DEVICES=1 python3 train/train_smac.py --env_name ${env} \
         --algorithm_name ${algo} --experiment_name ${exp} --map_name ${map} \
          --seed 1 --n_training_threads 1 --buffer_size 5000 --lr 5e-4 --batch_size 32 --use_soft_update \
           ...
           --use_global_all_local_state --gain 1 --share_hyper_network --add_other_act_to_cent
